...
fun eu(until: Proposition, then: Proposition) = EU(until, then)
fun eu(until: String, then: Proposition) = eu(Match(until), then)
...
data class Match(val inspection: String) : R0 {
    ...
    override fun <S> filter(g: Graph<S>, inspections: List<Inspection<S>>, labels: Collection<Label<S>>): List<S> =
...
interface Proposition {
    ...
    fun <State> filter(g: Graph<State>, inspections: List<Inspection<State>>, labels: Collection<Label<State>>): List<State>
}
...
private fun <State> Collection<Label<State>>.labeled(proposition: Proposition): Set<State> =
